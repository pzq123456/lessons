...
#### 简述比值植被指数(RVI)的特性
- 公式: $RVI=\frac{DN_R}{DN_{NIR}}$ or $RVI=\frac{\rho_R}{\rho_{NIR}}$
...
* 低植被覆盖度时（＜15%），植被的 NDVI 值高于裸士的 NDVI 值，植被可以被检测出来；但因植被覆盖度很低（如干旱、半干旱地区），其 NDVI 很难指示区域生物量；
* 中植被覆盖度时（25-80%），其 NDVI 值随生物量的增加呈线性迅速增加； 
* 高植被覆盖度时（＞80%），其NDVI值增加延缓而呈现饱和状态，对植被检测灵敏度下降。
...
## 计算题
> landset 8 传感器 波段节选
| Band Name | BandWidth | Resolution |
| --------- | --------- | ---------- |
| Band2 Blue | 0.45-0.51 |     30     |
| Band3 Green | 0.53-0.59 |   30     |
| Band4 Red | 0.64-0.67 |     30     |
| Band5 Near Infrared | 0.85-0.88 | 30 |
| Band6 Shortwave Infrared 1 | 1.57-1.65 | 30 |
| Band7 Shortwave Infrared 2 | 2.11-2.29 | 30 |
### 植被指数计算
#### NDVI
