...
* 植被:植被的反射波谱特征曲线规律明显而独特,主要分为三个波段.
  * 可见光(0.4~0.76μm)波段有一`小的反射峰`,位于0.55μm的`绿`光波段处,其两侧`蓝光`波段0.45μm和`红光`波段0.67μm处为两个`吸收带`.
  * 近红外(0.7~0.8μm)波段有一`反射陡坡`,在1.1μm附近有一峰值.
  * 中红外(1.3~2.5μm)波段受绿色植物`含水量`的影响,吸收率大增,反射率下降,在1.45μm、1.95μm和2.7μm为中心是水的吸收带,形成低谷.
